...
export async function runChatMode(agent: any, config: any, textInp: string) {
  ...
  try {
    ...
    // Create the message stream with the input
    const stream = await agent.stream(
      {
        messages: [{
          type: 'human',
    ...
    return {
      ...
      data: {
        ...
        toolsMessages,
        summary: {
          totalMessages: agentMessages.length + toolsMessages.length,
          agentMessages: agentMessages.length,
          toolMessages: toolsMessages.length
        }
      }
    };
